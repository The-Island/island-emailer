...
Emailer.prototype.notify = function (recipient, note, body, cb) {
  ...
  this.db.Keys.create({user_id: recipient._id}, _.bind(function (err, key) {
    if (err) return cb(err);
 ...
 * @function cb
 */
Emailer.prototype.reset = function (user, cb) {
  cb = cb || function(){};
  if (!this.baseURI) return cb('baseURI required');

  // Create a login key for this email.
  this.db.Keys.create({user_id: user._id}, _.bind(function (err, key) {
    if (err) return cb(err);
